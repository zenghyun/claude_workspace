...
class BucketListApp {
    ...
    createBucketItemHTML(item) {
        ...
        const checkboxClass = item.completed ? 'checkbox-btn checked' : 'checkbox-btn';
        const itemClass = item.completed ? 'bucket-item completed' : 'bucket-item';

        return `
            <div class="${itemClass} flex items-center gap-4">
                <!-- 체크박스 -->
                <button
                    class="flex-shrink-0 ${checkboxClass}"
                    onclick="app.handleToggle('${item.id}')"
                >
                    <span class="text-sm font-bold text-white">${checkIcon}</span>
                </button>

                <!-- 제목 -->
                <div class="flex-1">
                    <p class="text-lg ${completedClass} break-words font-medium">${this.escapeHtml(item.title)}</p>
